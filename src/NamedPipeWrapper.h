...
public:
    ...
    DWORD GetNamedPipeClientProcessId()
    {
        DWORD dwPID = -1;

        if(::GetNamedPipeClientProcessId(m_hPipe, &dwPID))
            return dwPID;

        return dwPID;
    }

    DWORD GetNamedPipeClientSessionId()
    {
        DWORD SID = 0;

        if(::GetNamedPipeClientSessionId(m_hPipe, &SID))
            return SID;
